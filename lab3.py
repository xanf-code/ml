...
def entropy(data):
    ...
    en = 0
    if(p_ex > 0):
        en = -(p_ex/float(total_ex)) * \
            (math.log(p_ex, 2)-math.log(total_ex, 2))
    if(n_ex > 0):
        en += -(n_ex/float(total_ex)) * \
            (math.log(n_ex, 2)-math.log(total_ex, 2))
...
def gain(en_s, data_s, attrib):
    ...
    gain = en_s
    for value in values:
        gain -= len(data_s.loc[data_s[attrib] == value]) / \
            float(len(data_s)) * entropy(data_s.loc[data_s[attrib] == value])
...
def decision_tree(data):
    ...
    if(len(data.columns) == 1):
        ...
        for value in values:
            root.branch[value] = decision_tree(
                data.loc[data[attr] == value].drop(attr, axis=1))
